...
const products: Product[] = [
  { id: 2, name: 'ISA Brown', description: 'La ponedora estrella...', image: '/gallina-1.jpg', characteristics: ['300+ huevos/año'], ageOptions: commonAgeOptions },
  { id: 3, name: 'Leghorn Blanca', description: 'Máxima eficiencia...', image: '/gallina-3.jpg', characteristics: ['320+ huevos/año'], ageOptions: commonAgeOptions },
  { id: 4, name: 'Hy-Line Brown', description: 'Cáscara extra fuerte...', image: '/hyline.jpg', characteristics: ['350+ huevos/año'], ageOptions: commonAgeOptions },
];
...
export default function Products() {
  ...
  const sendWhatsAppQuote = () => {
    ...
    let message = `📦 *COTIZACIÓN - AVÍCOLA AGAVES*\n\n`;
    cart.forEach(item => {
      ...
      <div className="container mx-auto px-4">
        <div className="grid md:grid-cols-3 gap-8" ref={cardsRef}>
          {products.map((product) => (
            <div key={product.id} className="product-card bg-white p-6 rounded-3xl shadow-lg">
              <img src={product.image} className="rounded-2xl mb-4 h-48 w-full object-cover" />
              <h3 className="font-bold text-xl mb-2">{product.name}</h3>
              <div className="bg-gray-100 p-2 rounded-lg mb-4 text-center font-bold">{getPriceForSelectedAge(product)}</div>
              <select 
              ...
              <button onClick={() => addToCart(product)} className="w-full bg-[#1e1e1e] text-white py-3 rounded-xl font-bold">
                Agregar al Pedido
              </button>
            </div>
          ))}
